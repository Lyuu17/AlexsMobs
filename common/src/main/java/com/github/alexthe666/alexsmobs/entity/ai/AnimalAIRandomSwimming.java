...
public class AnimalAIRandomSwimming extends WanderAroundGoal {
    ...
    protected Vec3d getWanderTarget() {
        ...
        if(this.mob.getRandom().nextFloat() < 0.3F){
            var vector3d = findSurfaceTarget(this.mob, xzSpread, ySpread * 2);
            if(vector3d != null){
                return vector3d;
            }
        }
        var vector3d = NoPenaltyTargeting.find(this.mob, xzSpread, ySpread);

        for(int i = 0; vector3d != null && !this.mob.getWorld().getBlockState(AMBlockPos.fromVec3(vector3d)).canPathfindThrough(this.mob.getWorld(), AMBlockPos.fromVec3(vector3d), NavigationType.WATER) && i++ < 15; vector3d = NoPenaltyTargeting.find(this.mob, 10, ySpread)) {
        }
        if(submerged && vector3d != null){
            if(!this.mob.getWorld().getFluidState(AMBlockPos.fromVec3(vector3d).up()).isIn(FluidTags.WATER)){
                vector3d = vector3d.add(0, -2, 0);
            }else if(!this.mob.getWorld().getFluidState(AMBlockPos.fromVec3(vector3d).up(2)).isIn(FluidTags.WATER)){
