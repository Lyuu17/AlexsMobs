...
public class AquaticMoveController extends MoveControl {
    ...
    public void tick() {
        if (this.entity.isTouchingWater() || entity instanceof EntityWarpedToad && entity.isInLava()) {
        ...
        if (this.state == State.MOVE_TO && !this.entity.getNavigation().isIdle()) {
            ...
            this.entity.setVelocity(this.entity.getVelocity().add(0.0D, this.entity.getMovementSpeed() * d1 * 0.6D, 0.0D));
        } else {
            this.entity.setMovementSpeed(0.0F);
        }
    }
