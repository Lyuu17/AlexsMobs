...
public class ItemHemolymphBlaster extends Item {
    ...
    public ItemHemolymphBlaster(Item.Settings properties) {
        super(properties);
    }
    ...
    public void usageTick(World worldIn, LivingEntity livingEntityIn, ItemStack stack, int count) {
        if(isUsable(stack)) {
            if (count % 2 == 0) {
                ...
                var blood = new EntityHemolymph(worldIn, livingEntityIn, !left);
                ...
                blood.shoot(vector3d.x, vector3d.y, vector3d.z, 1F, 3);
                if (!worldIn.isClient) {
