    ...
    private static class SpawnBiomeEntry {
        ...
        public boolean matches(RegistryEntry<Biome> biomeHolder, Identifier registryName) {
            if(type.isDeprecated()){
                AlexsMobs.LOGGER.warn("biome config: BIOME_DICT and BIOME_CATEGORY are no longer valid in 1.19+. Please use BIOME_TAG instead.");
                return false;
            }

            if (type == BiomeEntryType.BIOME_TAG) {
                if (biomeHolder.streamTags().anyMatch((biomeTagKey -> biomeTagKey.id() != null && biomeTagKey.id().toString().equals(value)))) {
                    return !negate;
                }
            } else {
