...
import com.github.alexthe666.alexsmobs.renderer.layer.LayerUnderminerItem;
...
//        net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new net.minecraftforge.client.event.RenderLivingEvent.Post<EntityUnderminer, EntityModel<EntityUnderminer>>(entityIn, this, partialTicks, matrixStackIn, bufferIn, packedLightIn));
        ...
        if (miningPos != null) {
            ...
            //FIXME forge
//            net.minecraftforge.client.model.data.ModelData modelData = entityIn.getWorld().getModelDataManager().getAt(miningPos);
//            MinecraftClient.getInstance().getBlockRenderManager().renderBreakingTexture(entityIn.getWorld().getBlockState(miningPos), miningPos, entityIn.getWorld(), matrixStackIn, vertexconsumer1, modelData == null ? net.minecraftforge.client.model.data.ModelData.EMPTY : modelData);
            matrixStackIn.pop();
        }
    }
