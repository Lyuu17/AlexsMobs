...
public class RenderCentipedeTail extends MobEntityRenderer<EntityCentipedeTail, AdvancedEntityModel<EntityCentipedeTail>> {
    private static final Identifier TEXTURE = new Identifier("alexsmobs:textures/entity/cave_centipede.png");

    public RenderCentipedeTail(EntityRendererFactory.Context renderManagerIn) {
        super(renderManagerIn, new ModelCaveCentipede<>(2), 0.5F);
    }

    @Override
    protected float getLyingAngle(EntityCentipedeTail centipede) {
        return 180.0F;
    }

    @Override
    protected void setupTransforms(EntityCentipedeTail entity, MatrixStack stack, float pitchIn, float yawIn, float partialTickTime) {
    ...
    @Override
    public Identifier getTexture(EntityCentipedeTail entity) {
        return TEXTURE;
    }
}
