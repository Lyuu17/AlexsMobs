...
public class ModelMungus extends AdvancedEntityModel<EntityMungus> {
	...
	public ModelMungus(float f) {
		...
		body.addBox(-6.0F, -16.0F, -4.0F, 12.0F, 16.0F, 8.0F, f, false);

		hair = new AdvancedModelBox(this, "hair");
		hair.setPos(0.0F, -16.0F, 0.0F);
		body.addChild(hair);
		hair.setTextureOffset(33, 0);
		hair.addBox(-5.0F, -5.0F, 0.0F, 10.0F, 5.0F, 0.0F, f, false);

		eye = new AdvancedModelBox(this, "eye");
		eye.setPos(0.0F, -11.0F, -4.1F);
		body.addChild(eye);
		eye.setTextureOffset(0, 0);
		eye.addBox(-1.0F, -1.0F, 0.0F, 2.0F, 2.0F, 1.0F, f, false);

		leg_left = new AdvancedModelBox(this, "leg_left");
		leg_left.setPos(3.0F, 0.0F, 0.0F);
		body.addChild(leg_left);
		leg_left.setTextureOffset(0, 39);
		leg_left.addBox(-2.0F, 0.0F, -3.0F, 5.0F, 7.0F, 6.0F, f, false);

		leg_right = new AdvancedModelBox(this, "leg_right");
		leg_right.setPos(-3.0F, 0.0F, 0.0F);
		body.addChild(leg_right);
		leg_right.setTextureOffset(0, 25);
		leg_right.addBox(-3.0F, 0.0F, -3.0F, 5.0F, 7.0F, 6.0F, f, false);

		nose = new AdvancedModelBox(this, "nose");
		nose.setPos(0.0F, -9.0F, -4.0F);
		body.addChild(nose);
		nose.setTextureOffset(35, 43);
		nose.addBox(-1.0F, 0.0F, -2.0F, 2.0F, 5.0F, 2.0F, f, false);

		sack = new AdvancedModelBox(this, "sack");
		sack.setPos(0.0F, -7.0F, 4.0F);
		body.addChild(sack);
		sack.setTextureOffset(23, 25);
		sack.addBox(-4.0F, -8.0F, 0.0F, 8.0F, 10.0F, 3.0F, f, false);

		this.updateDefaultPose();
	}

	@Override
	public void setAngles(EntityMungus entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		this.resetToDefaultPose();
		float walkSpeed = 0.7F;
		float walkDegree = 0.6F;
		float idleSpeed = 0.1F;
		float idleDegree = 0.1F;
		float swell = Math.min(entity.prevSwellProgress + (entity.swellProgress - entity.prevSwellProgress) * MinecraftClient.getInstance().getTickDelta(), 10F);
		float glowyBob = (swell * 0.22F) + 0.95F + (MathHelper.cos(ageInTicks * (0.1F + swell * 0.2F)) + 1F) * (0.05F + swell * 0.02F);
		var targetPos = entity.getBeamTarget();
		if(targetPos == null) {
			Entity look = MinecraftClient.getInstance().getCameraEntity();
			if (look != null) {
				var vector3d = look.getCameraPosVec(0.0F);
				var vector3d1 = entity.getCameraPosVec(0.0F);
				...
				var vector3d3 = (new Vec3d(vector3d1.x - vector3d.x, 0.0D, vector3d1.z - vector3d.z)).normalize().rotateY((MathHelper.PI / 2F));
			...
			var vector3d = Vec3d.ofCenter(targetPos);
			var vector3d1 = entity.getCameraPosVec(0.0F);
			double d0 = vector3d.y - vector3d1.y;
			if (d0 > 0.0D) {
				this.eye.rotationPointY = -11.0F;
